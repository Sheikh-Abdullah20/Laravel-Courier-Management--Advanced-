...
class ShipmentController extends Controller implements HasMiddleware
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->hasRole('admin')) {
            ...
            $search_tracking_number = $request->search_tracking_number;
            $agentSearch = $request->agents;
            $statusSearch = $request->status;

            // This Code is also Working But it Doesnt Have Track Search Functionality
    ...
    public function update(Request $request, string $id)
    {
        ...
        $shipment = Shipment::find($id);
        $agent = User::with('roles')->whereHas('roles', function ($query) {
            $query->where('name', 'agent');
        })->where('name', $shipment->agent->name)->pluck('email');
        ...
        if ($shipment && Auth::user()->hasRole('admin')) {
            ...
            if ($updated) {
                if ($request->approval === 'Approved') {
                    $mail = Mail::to($agent)->send(new shipmentapprovalMail($request->all(), $shipment));
                }
